...
std::string convertIp2String(UINT32 ipAddress)
{
	IN_ADDR address;
...
NATAddressInfo NATService::updateDnLoadMapInfo(const NatKeyInfo &keyInfo, const NATAddressInfo &dnatInfo, PPACKET_S packageInfo, bool isFind)
{
	...
	if (dnatMap.end() == iter)
	{
		if (!isFind)
		{
			...
			printNatInfo(keyInfo, dnatAddressInfo, false);
			dnatMap[keyInfo] = dnatAddressInfo;
		}
		else
		{
			LOGINFO(" dnat find key %s|%u|%s|%u|%u failed", convertIp2String(keyInfo.src_address).c_str(), ntohs(keyInfo.src_port), convertIp2String(keyInfo.dst_address).c_str(), ntohs(keyInfo.dst_port), keyInfo.protocol);
		}
	}
	else
	{
		...
		printNatInfo(keyInfo, defaultdnatAddressInfo, false);
...
void NATService::printNatInfo(const NatKeyInfo &key, NATAddressInfo &natAddress, bool isUpload)
{
	char srcBuff[500] = { 0 };
	char dstBuff[500] = { 0 };

	snprintf(srcBuff,sizeof(srcBuff), "%s|%u|%s|%u|%u", convertIp2String(key.src_address).c_str(), ntohs(key.src_port), convertIp2String(key.dst_address).c_str(), ntohs(key.dst_port), key.protocol);

	snprintf(dstBuff, sizeof(dstBuff), "%s|%u|%s|%u|%u", convertIp2String(natAddress.src_address).c_str(), ntohs(natAddress.src_port), convertIp2String(natAddress.dst_address).c_str(), ntohs(natAddress.dst_port), key.protocol);
	if(!natAddress.isPrintFlag)
	{
		if (isUpload)
		{
			LOGINFO("upload src %s ==> dst %s", srcBuff, dstBuff);
		}
		else
		{
			LOGINFO("dnload src %s ==> dst %s", srcBuff, dstBuff);
		}

		natAddress.isPrintFlag = true;
	}
}
...
NATAddressInfo NATService::updateUpLoadMapInfo(const NatKeyInfo &keyInfo, PPACKET_S packageInfo, uint32_t newSrcAddress, bool isIcmp)
{
	...
	if (snatMap.end() == iter)
	{
		printNatInfo(keyInfo, snatAddressInfo);
...
bool NATService::checkIpInNatList(uint8_t* ip_packet, uint32_t length)
{
	bool result = false;
	...
	{
		LOGINFO("ip package is %p or length %u", ip_packet, length);
	...
	switch (ip_v4_header->protocol)
	{
		...
		default:
		{
			LOGINFO("unrecognise ip src address %s ip dest address %s protocol %u", convertIp2String(ip_v4_header->src_address).c_str(), convertIp2String(ip_v4_header->dest_address).c_str(), ip_v4_header->protocol);
...
NatKeyInfo NATService::GetNatKeyFromIpTcpHeader(IPV4Header *ipHeader, TCPHeader *tcpHeader)
{
	...
	natInfo.protocol = ipHeader->protocol;
