...
namespace winfilter {
  namespace ClientEvent {
    ...
    enum {
      ...
      FATAL_ERROR_START    = TRANSPORT_ERROR,     // start of fatal errors
    };
	...
	"EPKI_INVALID_ALIAS",
	"RELAY_ERROR",
	"vpn tunnel ping timeout tunnel corrupt",
      };
	...
	struct UnknowError : public ReasonBase
	{
		UnknowError(std::string reason) : ReasonBase(UNKOWN_ERROR, std::move(reason)) {}
	};
