...
#include <winfilter/asio/asiowork.hpp>
#include <winfilter/error/excode.hpp>
#include <winfilter/time/asiotimer.hpp>
#include <winfilter/client/cliopt.hpp>
...
namespace winfilter {
  ...
  public:
    ...
    void set_transport_fiter(Client::TransportFilter* filter)
    {
      if (client) {
        client->set_transport_filter(filter);
      }
    }
    ...
    {
      if (client) {
        client->send_data_by_transport(buf);
      }
    }
      ...
      if (!halt)
	{
	  if (dont_restart_)
		...
		case Error::UNDEF: // means that there wasn't a fatal error
		{
			...
			ClientEvent::Base::Ptr ev = new ClientEvent::UnknowError(reason);
			client_options->events().add_event(std::move(ev));
			client_options->stats().error(Error::UNDEF);
			stop();
			//queue_restart();
			break;
		}
