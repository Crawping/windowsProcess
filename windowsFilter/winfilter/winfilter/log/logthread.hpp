...
# define OPENVPN_LOG(args) \
  do { \
    if (winfilter::Log::Context::defined()) {	\
      std::ostringstream _ovpn_log; \
      _ovpn_log << args << '\n'; \
      (winfilter::Log::Context::obj()->log(OPENVPN_LOG_INFO(_ovpn_log.str()))); \
    } \
  } while (0)
...
# define OPENVPN_LOG_STRING(str) \
  do { \
    if (winfilter::Log::Context::defined()) {			  \
      (winfilter::Log::Context::obj()->log(OPENVPN_LOG_INFO(str))); \
    } \
  } while (0)
...
#else
    static OPENVPN_EXTERN thread_local OPENVPN_LOG_CLASS* global_log; // GLOBAL
#endif
    struct Context
    {
