...
#include <winfilter/common/stringize.hpp> // for OPENVPN_STRINGIZE
...
# define OPENVPN_EXCEPTION(C) \
  class C : public winfilter::Exception { \
  public: \
    C() : winfilter::Exception(#C OPENVPN_FILE_LINE) {} \
    C(std::string err) : winfilter::Exception(#C OPENVPN_FILE_LINE ": " + err) {} \
  }
...
# define OPENVPN_UNTAGGED_EXCEPTION(C) \
  class C : public winfilter::Exception { \
  public: \
    C(std::string err) : winfilter::Exception(err) {} \
  }
...
# define OPENVPN_THROW_EXCEPTION(stuff) \
  do { \
    ...
    _ovpn_exc << stuff; \
    throw winfilter::Exception(_ovpn_exc.str()); \
  } while (0)
