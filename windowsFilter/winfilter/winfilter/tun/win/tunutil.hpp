...
#include <winfilter/common/socktypes.hpp> // prevent winsock multiple def errors
...
#include <winfilter/common/stringize.hpp>
#include <winfilter/common/action.hpp>
#include <winfilter/common/uniqueptr.hpp>
#include <winfilter/buffer/buffer.hpp>
#include <winfilter/addr/ip.hpp>
#include <winfilter/tun/builder/capture.hpp>
#include <winfilter/win/reg.hpp>
#include <winfilter/win/scoped_handle.hpp>
#include <winfilter/win/unicode.hpp>
#include <winfilter/win/cmd.hpp>
#include <winfilter/win/winerr.hpp>
	      ...
	      for (size_t i = 0; i < rt->dwNumEntries; ++i)
		{
		  ...
		  if (row->dwForwardIfIndex == index)
			...
			continue;
		      ...
		      actions.add(new WinCmd("netsh interface ip delete route " + net_str + '/' + winfilter::to_string(pl) + ' ' + winfilter::to_string(index) + " store=active"));
		    }
		}
	    }
	}
		  ...
		  if (row->InterfaceIndex == index)
		      ...
		      if (row->DestinationPrefix.Prefix.si_family == AF_INET6)
			{
			  ...
			  if ((net & ll_mask) == ll_net && pl >= 64)
			    continue;
			  actions.add(new WinCmd("netsh interface ipv6 delete route " + net_str + '/' + winfilter::to_string(pl) + ' ' + winfilter::to_string(index) + " store=active"));
			}
		    }
		}
	    }
