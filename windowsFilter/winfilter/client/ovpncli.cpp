...
#include <winfilter/init/initprocess.hpp>
#include <winfilter/common/size.hpp>
#include <winfilter/common/platform_string.hpp>
#include <winfilter/common/count.hpp>
#include <winfilter/asio/asiostop.hpp>
#include <winfilter/time/asiotimer.hpp>
#include <winfilter/client/cliconnect.hpp>
#include <winfilter/client/cliopthelper.hpp>
#include <winfilter/options/merge.hpp>
#include <winfilter/error/excode.hpp>
#include <winfilter/crypto/selftest.hpp>

// copyright
#include <winfilter/legal/copyright.hpp>
...
protected:
	  void event_code_convert(ClientEvent::Base::Ptr event, Event& ev)
	  {
		  ERROR_NUMBER code = FSM_SUCCESS;

		  switch (event->id())
		  {
			/*upload normal action*/
			case ClientEvent::DISCONNECTED:
			case ClientEvent::CONNECTED:
			case ClientEvent::RECONNECTING:
			case ClientEvent::RESOLVE:
			case ClientEvent::WAIT:
			case ClientEvent::WAIT_PROXY:
			case ClientEvent::CONNECTING:
			case ClientEvent::GET_CONFIG:
			case ClientEvent::ASSIGN_IP:
			case ClientEvent::ADD_ROUTES:
			case ClientEvent::ECHO_OPT:
			case ClientEvent::INFO:
			case ClientEvent::PAUSE:
			case ClientEvent::RESUME:
			case ClientEvent::RELAY:
				{
					break;
				}
			case ClientEvent::TRANSPORT_ERROR:
			case ClientEvent::TUN_ERROR:
			case ClientEvent::CLIENT_RESTART:
			case ClientEvent::CLIENT_HALT:
			case ClientEvent::CLIENT_SETUP:
			case ClientEvent::CONNECTION_TIMEOUT:
			case ClientEvent::EPKI_ERROR:
			case ClientEvent::EPKI_INVALID_ALIAS:
			case ClientEvent::RELAY_ERROR:
				{
					code = CONNECTING_ERROR;
					break;
				}
			case ClientEvent::AUTH_FAILED:
			case ClientEvent::CERT_VERIFY_FAIL:
			case ClientEvent::TLS_VERSION_MIN:
				{
					code = PASSWORD_ERROR;
					break;
				}
			case ClientEvent::INACTIVE_TIMEOUT:
				{
					code = INACTIVE_TIMEOUT;
					break;
				}
			case ClientEvent::TUN_SETUP_FAILED:
			case ClientEvent::TUN_IFACE_CREATE:
			case ClientEvent::TUN_IFACE_DISABLED:
				{
					code = TUNNEL_CREATE_ERROR;
					break;
				}
		  }
    ...
    // Add by lulei. @2018.11.08
    OPENVPN_CLIENT_EXPORT void OpenVPNClient::set_transport_filter(TrangsportFilter* filter)
    {
      this->state->session->set_transport_fiter(reinterpret_cast<ClientConnect::Client::TransportFilter*>(filter));
    }

    // Add by lulei. @2018.11.08
    OPENVPN_CLIENT_EXPORT void OpenVPNClient::send_data_by_transport(BufferAllocated& buf)
    {
      this->state->session->send_data_by_transport(buf);
    }
